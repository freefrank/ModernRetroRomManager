...
- 扫描并整理多平台 ROM 游戏库
- 支持按名称、文件大小或修改时间对 ROM 库排序,并可切换升序/降序
- 保存时自动整理多碟游戏:各碟收进同一子文件夹、生成 `.m3u` 播放列表,库与导出均折叠成一条,RetroArch/batocera 可换碟(识别 `(Disc N)`、`Disc A/B`、`CD1/CD2` 及 `第1碟` 等中文碟号)
- 管理多个相互独立的游戏库，支持改名，并可从设置页或 ROM 侧栏切换当前游戏库
- 使用即时持久索引、按系统延迟加载和非阻塞后台增量扫描，快速打开本地、SD 卡及 Samba 大型游戏库
...
- 批量抓取可选 AI 名称解析兜底：本地无法识别英文名的汉化 ROM 交给 LLM 在内置 No-Intro 标题清单内匹配，结果本地缓存
- 可按 EmulationStation 或 Pegasus 格式导出当前或指定游戏库，支持文件级进度、实时速度、停止、同大小文件跳过、仅导出 ROM 与资产，并在复制前预检目标磁盘可用空间、空间不足时中止
- 在支持日志级别筛选的底部 Console 中查看扫描、抓取、Provider 和错误信息;导出与单向同步决策还会写入日志文件,设置→关于页可一键「打开日志目录」
- ROM 库右键菜单支持打开文件位置、隐藏/取消隐藏、永久删除；隐藏的 ROM 不显示也不导出同步
- 导出可选单向同步，删除目标目录中已隐藏/删除的 ROM（BIOS 始终保留），使目标与库一致
...
AI Metadata 翻译可单独配置端点、API Key、模型、目标语言、思考强度和批量上下文上限。请求由原生后端发送；不完整的结构化响应会自动拆分重试，按语言完成标记可避免重复翻译已有内容。
