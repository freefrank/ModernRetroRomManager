...
export default function Library() {
  ...
  const filteredRoms = useMemo(() => {
    ...
    if (selectedSystem) {
      ...
      currentRoms = systemData?.roms || [];
    } else {
      currentRoms = systemRoms.flatMap(s => s.roms);
    }

    // 2. 应用搜索过滤
    if (debouncedSearch) {
      const lowerQuery = debouncedSearch.toLowerCase();
      return currentRoms.filter(r => r.name.toLowerCase().includes(lowerQuery));
    }

    return currentRoms;
  }, [systemRoms, selectedSystem, debouncedSearch]);
