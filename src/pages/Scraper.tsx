...
export default function Scraper() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* 工具栏 */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-default bg-[#0B0C15]/50 backdrop-blur-md sticky top-0 z-10">
        <h1 className="text-xl font-bold text-white">{t("scraper.title")}</h1>
      </div>

      {/* API 配置 */}
      <div className="flex-1 p-6 overflow-auto">
        <div className="max-w-3xl">
          <h2 className="text-lg font-medium mb-4 text-white">{t("scraper.apiConfig.title")}</h2>
          ...
          <div className="space-y-4">
            {providers.map((p) => {
              ...
              }[p.color] || "bg-white/10 text-white";

              return (
                <div key={p.id} className="p-4 bg-[#151621] border border-white/5 rounded-xl hover:border-white/10 transition-colors">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className="w-11 h-6 bg-white/10 rounded-full peer peer-checked:bg-accent-primary peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"></div>
                    </label>
                  </div>
                      ...
                      {p.fields.map((field) => (
                        <div key={field.key}>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 bg-[#0B0C15] border border-white/10 rounded-lg focus:outline-none focus:border-accent-primary text-sm text-white"
                          />
                        </div>
                      ))}
                    </div>
