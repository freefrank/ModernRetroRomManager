...
pub fn smart_cn_similarity(query: &str, target: &str) -> f32 {
    ...
    // [Fix] 如果任意一方的核心词为空（说明没有有效的中文字符），直接返回 0.0
    // 除非原始字符串完全相等（这种情况在 exact match 阶段已处理）
    // 这防止了两个纯英文/纯数字字符串（如 "FIFA" 和 "FIFA"）在中文模糊匹配算法中得到 1.0 的高分
    if query_cn_core.is_empty() || target_cn_core.is_empty() {
        return 0.0;
    }
...
impl ScraperProvider for LocalCnProvider {
        ...
        for entry in entries {
            ...
            if chinese_lower == query_name_lower || chinese_lower == query_file_stem {
                results.push(SearchResult {
                    ...
                    source_id: format!("{}|{}", system, entry.english_name), // Encode system in ID
    ...
    async fn get_metadata(&self, source_id: &str) -> Result<GameMetadata, String> {
        ...
        // 如果我们有 system 信息，直接去对应 system 查找 (O(1))
        ...
        // Fallback: 如果没有 system 或者查找失败，遍历所有缓存 (Legacy behavior)
